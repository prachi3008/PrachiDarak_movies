...
import com.example.moviesapp.domain.interactor.*
import com.example.moviesapp.model.MovieDetail
import com.example.moviesapp.model.toSimple
...
class MovieDetailsViewModel : ViewModel() {
    ...
    fun addFavoriteMovie(context: Context, movie: MovieDetail) {
        ...
        disposable =
            AddFavoriteMovieUseCase(MoviesLocalRepositoryImpl(context)).execute(movie.toSimple())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
    ...
    fun deleteFavoriteMovie(context: Context, movie: MovieDetail) {
        ...
        disposable =
            DeleteFavoriteMovieUseCase(MoviesLocalRepositoryImpl(context)).execute(movie.toSimple())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
    ...
    fun updateFavoriteMovie(context: Context, movie: MovieDetail) {
        ...
        disposable =
            UpdateFavoriteMovieUseCase(MoviesLocalRepositoryImpl(context)).execute(movie.toSimple())
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe({
                    favoritesStateFlow.value = Resource.success(true)
